...
maxintensity <- 127
num <- 1
for (i in seq(1,nrow(mysample$data[,series]),by=1)){
...
for (j in (1:nrow(pixels))){
  m[as.integer(pixels[j,"x"]),as.integer(pixels[j,"y"])] <-1- (as.integer(pixels[j,"value"])-minintensity)/(maxintensity-minintensity)
}
