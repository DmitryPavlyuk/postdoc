...
estimateFeaturesGlobal(fs.function=fsMTS::fsMTS, 
                 ...
                 max.lag = 3,
                 ta=ta,
                 fs.folder = file.path(dir,"fs",length(series),"LARSg"),
...
xModel.star$run(dat[1:1440,series], forecastingSteps=3, arLags=3, 
                fs.folder=file.path(dir,"fs",length(series),"CCFg",12,3), 
                control=list(),
                returnModel=F, 
                refine = F, 
                include.mean=F, 
...
validationSize <- nrow(sampl$data)-12*60*14/ta
validationEnd <- nrow(sampl$data)
forecastEvery<-1
forecastingSteps<-12

# Define a grid for tuning parameters
cvgrid <- expand.grid(trainingMinutes = c(12*60*14),
                      include.mean = c(F),
                      arLags = c(3),
                      stationary =  c(T),
                      allowdrift =  c(F),
...
results <- readRDS("c:/tmp/baseline.results.rds")
...
pres<-results%>%filter(model=="STAR ",forecast_horizon==12)%>%mutate(dow_time=format(last_date,"%w %H:%M"))

p1<-pres%>%group_by(dow_time)%>%summarise(mean.mae=mae(actual,forecasted))%>%arrange(dow_time)
p1%>%ggplot(aes(x = dow_time, y=mean.mae, group = 1)) + geom_line()

p2<-pres%>%group_by(detector)%>%summarise(mean.mae=mae(actual,forecasted))%>%arrange(detector)
p2%>%ggplot(aes(x = detector, y=mean.mae, group = 1)) + geom_line()
...
p1$model<-"VAR"
t<-bind_rows(tmp, p1)
t%>%ggplot(aes(x = dow_time, y=mean.mae, group = model, col=model)) + geom_line()
t%>%group_by(model)%>%summarise(mean.mae = mean(mean.mae))


 results.cum <-results%>%
    mutate(modelid=gsub(" NA","",paste(model,trainingMinutes, arLags, 
                         include.mean)))%>%
...
results.cum.filtered<-results.cum%>%filter(forecast_horizon==3)
