...
validationSize <- nrow(sampl$data)-24*60*3/ta
...
cvgrid <- expand.grid(trainingMinutes = c(24*60*3),
                      include.mean = c(F),
                      arLags = c(maxlag),
                      stationary =  c(T),
                      allowdrift =  c(F),
...
reslist<-list()
for (f in list.files("thres")){
  reslist[[length(reslist)+1]]<-readRDS(file.path("thres", f))
}
