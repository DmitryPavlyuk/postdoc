...
validationSize <- nrow(data)-2*week/ta
...
s%>%filter(forecast_horizon<13)%>%ungroup%>%mutate(key=paste(model,trainingMinutes,fs.folder))%>%select(key,threshold,forecast_horizon,MAE)%>%
  spread(key=forecast_horizon, value=MAE)%>%print(n=24)


r<-results%>%mutate(fs.folder = gsub("TRC2020/fs/64/","",fs.folder))
r<-r%>%filter(forecast_horizon==6,wday(datetime)==3)
r<-r%>%mutate(fs.folder = gsub("/36/3","",fs.folder))
r<-r%>%mutate(fs.folder = gsub("/0.3","",fs.folder))
r<-r%>%mutate(fs.folder = gsub("/0.1","",fs.folder))
r<-r%>%mutate(fs.folder = gsub("/20160/3/2017-10-08_000000.rds","",fs.folder))
r<-r%>%mutate(fs.folder = gsub("/20160/3/0.2/2017-10-08_000000.rds","",fs.folder))
r<-r%>%mutate(fs.folder = gsub("/4032/3/2017-08-13_000000.rds","",fs.folder))
r<-r%>%mutate(dow_time=format(datetime, "%H:%M", tz="GMT"))
rt<-r%>%group_by(fs.folder, dow_time)%>%summarise(mae=mae(actual,forecasted),ld=last(datetime))
rt%>%ggplot(aes(x = ld, y = mae, col=fs.folder, group=fs.folder)) + geom_line(size=1) +
...
rt%>%group_by(fs.folder)%>%summarise(sd(mae))

rt2<-r%>%group_by(fs.folder, detector)%>%summarise(mae=mae(actual,forecasted))
rt2%>%group_by(fs.folder)%>%summarise(sd(mae))


rt%>%spread(key=fs.folder,value=mae)%>%
    mutate(`CCF vs. EnsVoting`=`CCF`-`EnsVoting`)%>%
    mutate(`MI vs. EnsVoting`=`MI`-`EnsVoting`)%>%
    mutate(`LARS vs. EnsVoting`=`LARS`-`EnsVoting`)%>%
    mutate(`RF vs. EnsVoting`=`RF`-`EnsVoting`)%>%
    mutate(`GLASSO vs. EnsVoting`=`GLASSOloc`-`EnsVoting`)%>%
    mutate(`EnsRanking vs. EnsVoting`=`EnsRanking`-`EnsVoting`)%>%
    gather(`CCF vs. EnsVoting`,`MI vs. EnsVoting`,`LARS vs. EnsVoting`,`GLASSO vs. EnsVoting`,`EnsRanking vs. EnsVoting`,`RF vs. EnsVoting`,key="fs.folder", value="Difference")%>%
    ggplot(aes(x = ld, y = Difference, col=fs.folder, group=fs.folder)) + 
