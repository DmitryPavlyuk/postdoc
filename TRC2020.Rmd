...
cvgrid <- expand.grid(trainingMinutes = c(2*week),
                      include.mean = c(F),
                      arLags = c(maxlag),
                      stationary =  c(T),
                      allowdrift =  c(F),
                      fs.folder=c(file.path(dir,"fs",length(series),"GLASSOloc",20160,maxlag,"0.0005","2017-10-08_000000.rds")),
                      threshold=c(0.9),
                      ownlags=c(T),
                      kneighbours=c(36))
rgrid<-NULL
rgrid <- bind_rows(rgrid,cvgrid)
source(file.path("R","models_spvar.R"))
source(file.path("R","cv_functions.R"))
# Run all models
RunModels(data, series, ta, forecastingSteps, forecastEvery,
                      validationSize, validationEnd, 
                      lagMatrix, shortestA, rgrid,
                      results.folder="thtrain-hourly-glasso")
 

test<-readRDS(file.path(dir,"fs",length(series),"GLASSOloc",20160,maxlag,0.0005,"2017-10-08_000000.rds"))
fsSparsity(test)

#GLASSO sparsity
#rho = 0.0005 - 0.8774414
#rho = 0.001 - 0.7918294
#rho = 0.002 - 0.6700033
#rho = 0.005 - 0.4963379
#rho = 0.02 - 0.2907715
...
#rho = 0.37 - 0.1051432
#rho = 0.43 - 0.05037435
bind_rows(unlist(rgrid))
for (r in 1:nrow(rgrid)){
    cv <- rgrid[r,]
  print(cv)
}
```
...
reslist<-list()
results<-NULL
fold<-file.path(dir,"thtrain-hourly","global","STAR")
...
s%>%ungroup%>%mutate(key=paste(model,trainingMinutes,fs.folder))%>%select(key,threshold,forecast_horizon,MAE)%>%spread(key=threshold, value=MAE)%>%filter(forecast_horizon==3)%>%print(n=34)
