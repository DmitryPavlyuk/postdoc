...
sample.rds <- file.path(data.folder.prepared,"ALL5.rds")
...
#  series<-paste0(names(res),".volume")
 ...
 #series<-paste0(sample(config.nodes%>%select(node_name)%>%pull, 200, replace=F),".volume")
  ...
  #dat.training%>%filter(node=="rnd_95345.volume", datetime>"2017-07-31 06:55:00")%>%ggplot(aes(x = datetime, y = prepared, col=node, group=node)) + geom_line()+scale_x_datetime(labels = date_format("%H:%m"), date_breaks ="12 hours")
