...
cl <- makeCluster(7, outfile="outfile.txt")
registerDoParallel(cl)
foreach (d = 1:length(days),.export=c("loadDay"),
                  .packages=c("fsMTS","dplyr")) %dopar% {
  res<-list()
  day<-days[d]
  for (fold in c(CCF.folder,MI.folder,GLASSO.folder,LARS.folder,RF.folder,
     CCF.folder.drift,MI.folder.drift,GLASSO.folder.drift,LARS.folder.drift,RF.folder.drift)){
  ...
  print(filename)
  saveRDS(result, file.path("stability", filename))
}

te<-readRDS(file.path("stability","2017-07-31_000000.rds"))
image(msim)
mean(msim)
max(msim)
